...
from app.crud.school import get_school, get_schools, create_school, delete_school, update_school
...
def read_schools(skip: int = 0, limit: int = int(os.getenv("PAGINATION_DEFAULT_LIMIT", 10)), db: Session = Depends(get_db)):
...
def update_school_endpoint(school_id: uuid.UUID, school: SchoolCreate, db: Session = Depends(get_db)):
