...
from app.crud.payment_type import get_payment_type, get_payment_types, create_payment_type, delete_payment_type, update_payment_type
...
def update_payment_type_endpoint(payment_type_id: uuid.UUID, payment_type: PaymentTypeCreate, db: Session = Depends(get_db)):
    db_payment_type = update_payment_type(db=db, payment_type_id=payment_type_id, payment_type_update=payment_type)
    if db_payment_type is None:
        raise HTTPException(status_code=404, detail="Payment type not found")
    return db_payment_type
